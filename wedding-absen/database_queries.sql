...
SELECT id, nama_tamu, alamat_tamu, hadir, is_generated, checkin, created_at
FROM data_tamu
...
SELECT 
  ...
  COUNT(CASE WHEN hadir = false OR hadir IS NULL THEN 1 END) as belum_checkin
FROM data_tamu;
...
SELECT nama_tamu, alamat_tamu, checkin
FROM data_tamu
WHERE hadir = true
