...
export function Hero({ content = defaultWeddingContent }: { content?: WeddingContent }) {
  const section = getWeddingSection(content, "hero");
  const title = section.title || `The Wedding of ${content.bride.shortName} and ${content.groom.shortName}`;
  return (
    <section id="home" className="noise relative min-h-[100svh] overflow-hidden">
      <Image src={section.background || content.images.hero} alt={`Potret ${content.shortNames}`} fill priority sizes="100vw" className="object-cover object-[50%_52%] md:object-[50%_47%]" />
      ...
      <div className="page-shell relative z-10 flex min-h-[100svh] flex-col justify-end pb-24 pt-24 md:justify-center md:pb-0">
        <h1 className="font-display max-w-4xl break-words leading-[.9]" style={{ fontSize: section.titleFontSize }}>{title}</h1>
        <div className="mt-7 flex items-center gap-4" aria-label="Tagar pernikahan">
          <span className="h-px w-10 bg-[#c5a059]/70" />
          <p className="font-display italic text-[#e9c176]" style={{ fontSize: section.bodyFontSize }}>{section.body || content.hashtag}</p>
        </div>
      </div>
    </section>
  );
