...
import type { CmsGalleryItem, CmsContent } from "@/lib/cms";
import { gallery as fallbackGallery } from "@/lib/wedding-config";
import { Reveal } from "./Reveal";
import { SectionBackdrop } from "./SectionBackdrop";

export function GallerySection({ cms, gallery }: { cms?: CmsContent; gallery?: CmsGalleryItem[] }) {
  const items = (gallery && gallery.filter(g=>g.active).length ? gallery.filter(g=>g.active) : fallbackGallery.map((g,i)=>({ id: String(i), src: g.src, alt: g.alt, active:true })) ) as CmsGalleryItem[];
  const [active, setActive] = useState<number | null>(null);
  const change = (direction: number) => setActive((current) => current === null ? 0 : (current + direction + items.length) % items.length);
        ...
        {items.map((photo, index) => (
          <Reveal key={photo.src} delay={(index % 4) * .04} className={index === 0 || index === 7 || index === 12 ? "col-span-2 row-span-2" : ""}>
            <button onClick={() => setActive(index)} aria-label={`Buka foto ${index + 1}`} className={`group relative block w-full overflow-hidden rounded-[4px] bg-[#1b1c18] ${index === 0 || index === 7 || index === 12 ? "aspect-[4/5]" : "aspect-[3/4]"}`}>
              <Image src={photo.src} alt={photo.alt} fill sizes="(max-width: 768px) 50vw, 25vw" unoptimized={photo.src.startsWith("data:") || photo.src.startsWith("http")} className="object-cover transition duration-700 group-hover:scale-105" />
      ...
      <AnimatePresence>
        {active !== null && (
          <motion.div role="dialog" aria-modal="true" aria-label="Galeri foto" className="fixed inset-0 z-[70] flex items-center justify-center bg-[#080906]/95 p-4 backdrop-blur-md" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setActive(null)}>
            ...
            <motion.div key={active} className="relative h-[82vh] w-[min(90vw,680px)]" initial={{ opacity: 0, scale: .97 }} animate={{ opacity: 1, scale: 1 }} onClick={(event) => event.stopPropagation()}><Image src={items[active].src} alt={items[active].alt} unoptimized={items[active].src.startsWith("data:") || items[active].src.startsWith("http")}  fill sizes="90vw" className="object-contain" /></motion.div>
