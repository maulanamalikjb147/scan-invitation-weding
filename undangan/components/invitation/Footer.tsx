...
export function Footer({ content = defaultWeddingContent }: { content?: WeddingContent }) {
  const section = getWeddingSection(content, "footer");
  return (
    <footer className="relative min-h-[78svh] overflow-hidden">
      <Image src={section.background || content.images.footer} alt={content.shortNames} fill sizes="100vw" className="object-cover object-center" />
      ...
      <div className="page-shell relative z-10 flex min-h-[78svh] flex-col items-center justify-end pb-28 text-center">
        <p className="font-display break-words text-[#e9c176]" style={{ fontSize: section.titleFontSize }}>{section.title || "Terima kasih"}</p>
        <p className="mt-8 max-w-xl leading-7 text-[#d1c5b4]" style={{ fontSize: section.bodyFontSize }}>{section.body || "Merupakan suatu kebahagiaan dan kehormatan bagi kami, apabila Bapak/Ibu/Saudara/i, berkenan hadir dan memberikan do'a restu kepada kami."}</p>
