...
const FALLBACK_DATE = "2026-09-26T08:00:00+07:00";




function getTimeLeft(dateStr: string) {
  const distance = Math.max(0, new Date(dateStr).getTime() - Date.now());
  return {
...
export function EventSection({ cms }: { cms?: CmsContent }) {
  const dateStr = cms?.date || FALLBACK_DATE;
  ...
  const ev = cms?.events?.[0] || { type: "Akad Nikah & Resepsi", time: "15.00 – 22.00 WIB", venue: "Rumah Makan Saung Gunung Jati", address: "Tasikmalaya, Jawa Barat" };
  const mapsUrl = cms?.mapsUrl || "https://maps.app.goo.gl/QNSAzPPbZwcx58Qd9";
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => setCountdown(getTimeLeft(dateStr)), 1000);
        ...
        <Reveal className="mx-auto mt-14 max-w-4xl rounded-[8px] border border-white/10 bg-[#f4f1ea]/95 p-5 text-center text-[#251d18] shadow-2xl md:p-8" delay={.16}>
          ...
          <div className="mt-4 grid items-center gap-4 md:grid-cols-[1fr_auto_1fr] md:gap-6">
            <div className="border-y border-[#251d18]/55 py-3 md:text-right">
              <p className="font-label text-[11px] font-semibold uppercase tracking-[.2em] text-[#251d18]/80">{ev.type}</p>
              <p className="font-display mt-1 text-lg text-[#6f5a31]">{ev.time}</p>
            </div>
            ...
            <div className="border-y border-[#251d18]/55 py-3 md:text-left">
              ...
              <p className="font-display mt-1 text-lg text-[#6f5a31]">{ev.venue}</p>
            </div>
          </div>
          <p className="font-display mt-5 text-xl capitalize text-[#6f5a31] md:text-2xl">{eventMonthYear}</p>
          <div className="mx-auto mt-7 flex max-w-xl justify-center gap-3 text-sm leading-6 text-[#4b4036]"><MapPin size={18} className="mt-1 shrink-0 text-[#9b7543]" /><p>{ev.venue}<br />{ev.address}</p></div>
        </Reveal>
        <Reveal className="mt-8 flex justify-center"><a href={mapsUrl} target="_blank" rel="noreferrer" className="font-label inline-flex min-h-12 items-center gap-3 rounded-[4px] border border-[#c5a059] px-7 text-xs font-semibold uppercase tracking-[.16em] text-[#e9c176] transition hover:bg-[#c5a059] hover:text-[#131410]"><Navigation size={16} /> Lihat lokasi</a></Reveal>
      </div>
    </section>
  );
}
